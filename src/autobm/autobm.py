...
from .model.pipeline import train_model, test_model, visualize_test 
from .utils import execute_code_and_get_model, code_impl, direct_construct
...
class AutoBM:
    ...
    def _get_model(self, policy: str) -> Optional[ModelElement]:
        try:
            ...
            model_class = execute_code_and_get_model(code)                
            model_0 = model_class()
            trained_model, _= train_model(model_0, self.train, self.val)
            test_results = test_model(trained_model, self.val)
            ...
            return ModelElement(policy, code,trained_model.state_dict(), loss, judgment or "N/A") # MSE and vis removed from constructor
